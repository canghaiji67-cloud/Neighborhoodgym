...
当前项目定位为**毕业设计原型系统**。本 README 同时作为项目说明文档与开发需求规格文档，用于指导 AI 或开发者完成完整系统实现。
...
如果将本 README 交给 AI 进行开发，请按以下优先级实现：

1. **智能合约层**：实现 `FitToken`、`MembershipManager`、`CheckIn`、`AchievementBadge`，并完成 Hardhat 部署脚本和单元测试。
2. **后端基础层**：实现 Express 项目结构、环境变量配置、Sequelize 数据模型、数据库迁移、JWT 鉴权和统一错误处理。
3. **链上交互服务**：实现后端 `blockchainService`，用于读取合约状态、校验交易哈希、调用服务钱包执行 NFT 铸造和奖励发放。
4. **业务 API 层**：实现用户认证、会员、课程、预约、打卡记录、成就、后台管理等 REST API。
5. **前端页面层**：实现钱包连接、登录注册、会员中心、课程预约、打卡、成就展示、后台管理等页面。
6. **联调与演示**：完成本地 Geth、MySQL、后端、前端的完整启动流程，并保证核心业务流程可演示。

开发时应优先保证核心闭环可运行，不要一开始实现复杂的事件监听、动态 Pinata 上传或生产级部署能力。
